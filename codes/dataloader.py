        ...
        while negative_sample_size < self.negative_sample_size:
            ...
            if self.mode == 'head-batch':
                ...
                if self.viable_neg is None:
                    ...
                    # if len(self.viable_neg[head]) == 0:
                    #     # last node as dummy node
                    #     negative_sample_list = [np.array([self.nentity-1])] * self.negative_sample_size
                    #     break
                    ...
                    # print(head, np.array(list(self.viable_neg[str(head)])), negative_sample[mask])
                    # print(head, (head, negative_sample[mask]))
            else:
...
class TrainDatasetDualNegative(TrainDataset):
    ...
    def __getitem__(self, idx):
        ...
        while negative_sample_size < self.negative_sample_size:
            ...
            # FIXME: hack way
            if str(head) in self.viable_neg.keys():
                array = np.array(list(self.viable_neg[str(head)]))
            else:
                array = np.array(list(self.viable_neg[str(15368)]))
        ...
        negative_sample_descendants = torch.from_numpy(negative_sample)
        ...
        negative_sample_siblings = torch.from_numpy(negative_sample)
        ...
        negative_sample = [negative_sample_descendants, negative_sample_siblings]
        if self.degree is None:
            return positive_sample, negative_sample, subsampling_weight, self.mode
        else:
            raise NotImplementedError
    ...
    def collate_fn(data):
        ...
        negative_sample = [torch.stack([_[1][0] for _ in data], dim=0),
                           torch.stack([_[1][1] for _ in data], dim=0)]
        subsample_weight = torch.cat([_[2] for _ in data], dim=0)
        mode = data[0][3]
        if len(data[0]) == 5:
            degree = torch.stack([_[4] for _ in data], dim=0)
